...
const ListAll = props => {
  ...
  const getTotal = () => {
    Axios.post(authUser.api_url + '/edit/count', { request_status: 2 })
      .then(ans => {
        if (ans.data.status) {
  ...
  const getData = data => {
    return new Promise((resolve, reject) => {
      try {
        Axios.post(authUser.api_url + '/edit/query', { ...data, request_status: 2 })
          .then(ans => {
            if (ans.data.status) {
            ...
            exportMenu: [
              {
                ...
                exportFunc: (cols, datas) =>
                  ExportPdf(cols, datas, 'Rejected Update Requests ' + moment().format('DD-MM-YYYY')),
              },
              {
                label: 'Export CSV',
                exportFunc: (cols, datas) =>
                  ExportCsv(cols, datas, 'Rejected Update Requests ' + moment().format('DD-MM-YYYY')),
              },
            ],
