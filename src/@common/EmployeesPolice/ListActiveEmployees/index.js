...
const ListAll = props => {
  ...
  const getData = data => {
    return new Promise((resolve, reject) => {
      try {
        Axios.post(authUser.api_url + '/emp/query', { ...data, is_employeed: 1 })
          .then(ans => {
            if (ans.data.status) {
  ...
  const editRowClick = async (event, rowData) => {
    ...
    setTimeout(() => {
      setDialogState(prevState => ({ ...prevState, show: true, rowId: rowData.id }));
    }, 10);
  };
        ...
        <MaterialTable
          ...
          options={{
            ...
            exportMenu: [
              {
                label: 'Export PDF',
                exportFunc: (cols, datas) => ExportPdf(cols, datas, 'Active Employees ' + moment().format('DD-MM-YYYY')),
              },
              {
                label: 'Export CSV',
                exportFunc: (cols, datas) => ExportCsv(cols, datas, 'Active Employees ' + moment().format('DD-MM-YYYY')),
              },
            ],
