...
const EndDialog = ({ dialogState, setDialogState }) => {
    // dialogState
    ...
    const validate = () => {
        var { username, full_name, email, cnic, new_password, contact } = formState

        var values = { username, full_name, cnic, contact }
        ...
        if (!validator.isLength(cnic, { max: 13, min: 13 })) {
            showMessage('error', 'Invalid CNIC Number It Should Be 13 Digit Number Without Dashes');
            return false;
        }
    ...
    const submitRequest = (data) => {
        try {
            Axios.post(authUser.api_url + '/user/update', data).then(result => {
                result = result.data;;
                if (result.status) {
    ...
    const onSubmit = (e) => {
        ...
        if (validate()) {
            try {
                ...
                const {
                    id,
                    username, full_name, cnic, email, new_password, contact,
                    criminal_notification, one_to_one, verisys, skip_witness, add_emp, update_emp
                } = formState
                const dataToSubmit = {
                    id,
                    username, full_name, cnic, email, new_password, contact,
                    criminal_notification, one_to_one, verisys, skip_witness, add_emp, update_emp,
                    is_newPassword: new_password.trim().length > 0 ? true : false
                };
                    ...
                    <CmtCardContent >
                        ...
                        <form autoComplete="off" onSubmit={onSubmit}>
                            <Box mb={2}>
                                ...
                                <Divider />
                                <br />
                                <PermissionsSelect state={formState} handleChange={handlePermissionChange} />
                                <br />
                                <Divider />
