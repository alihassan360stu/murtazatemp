...
const ListAll = (props) => {
  ...
  const columns = [
    ...
    {
      title: 'Executed By', field: 'full_name', render: (rowData) => {
        return (
          <div style={{ display: 'flex' }}>
            <h5>{rowData.full_name}</h5>
          </div>
        )
      }
    },
  ...
  const getData = (params) => {
    return new Promise((resolve, reject) => {
      if (testId) {
        Axios.post('test/detail', { ...params, test_id: testId, type: dateStatus, browser: browserTF, result_status: resultStatus, status: 1 }).then(ans => {
          if (ans.data.status) {
            ...
            for (let i = 0; i < tempData.history.length; ++i) {
              tempData.history[i].index = (skip + (i + 1));
            }
            setData(tempData);
            // if (tInfo == null) setTinfo(ans.data.data[0]);
  ...
  const actions = [
    row => ({
      icon: () => <BiCommentDots style={{ color: row.notes && row.notes.length > 0 ? orange[500] : blue[500] }} />,
      className: classes.actionBlueButton,
      tooltip: row.notes && row.notes.length > 0 ? `${row.notes[0].description}` : 'Add Comments',
  ...
  return (
    <PageContainer heading="" breadcrumbs={breadcrumbs}>
      <div style={{ marginTop: "-6%" }}>
        ...
        <Divider />
        <br />
        <Chart testId={testId} type={dateStatus} browser={browserTF} result_status={resultStatus} />
        <Divider />
        <br />
        <br />
