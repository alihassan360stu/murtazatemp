...
export default function ReactVirtualizedTable({ hideDialog, setRefereshData, selectedRowData, sourceId }) {
  ...
  const submitRequest = (data) => {
    try {
      Axios.post('test/change-group', data).then(result => {
        result = result.data;
        console.log(result)
        if (result.status) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (selectedRowData.name.icon === 1) {
        dataToSubmit = {
          source_id: selectedRowData._id,
          destination_id: group_id,
          type: selectedRowData.name.icon,
