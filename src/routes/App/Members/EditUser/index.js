...
const EditDialog = ({ hideDialog, setRefereshData, formdata }) => {
  // dialogState
  ...
  const [formState, setFormState] = useState({
    ...
    org_id:formdata.organization[0]._id
  });
  ...
  const submitRequest = (data) => {
    try {
      Axios.put('user', data).then(result => {
        result = result.data;;
        if (result.status) {
  ...
  const onSubmit = (e) => {
    ...
    if (validate()) {
      try {
        let { username,full_name, email,type,user_id,org_id } = formState
        setFormState(prevState => ({ ...prevState, is_loading: true }))
        if (org) {
          let dataToSubmit = { username,full_name, email,type,user_id,org_id  };
